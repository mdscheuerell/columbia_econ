...
Combining equations (\ref{N_ricker}) and (\ref{H_sar_simple_log}) gives

\begin{equation} \label{full_model}
\log(N^A_{i,t}) = \left[ \log (\alpha_{i,t}) + \log(N^A_{i,t-k}) - \beta_N N^A_{i,t-k} \right] + \left[ \log (s^H_{i,t-k}) + \log (H^J_{i,t-k}) \right] + w_{i,t}
\end{equation}
...
Here we can make the second assumption that the smolt-to-adult survival (SAR) of hatchery fish is known from independent studies and therefore fix the $\log (s^H_{i,t-k})$ term in equation (\ref{full_model}) rather than estimate it.
...
\begin{equation} \label{cov_effects_full}
\log (\alpha_{i,t}) = \beta_0 + \beta_{F,i} F_{i,t-l} + \beta_{O,i} O_{i,t-k} + \beta_{S,i} S_{i,t-m}.
\end{equation}

Importantly, here the covariates are not log-transformed, as it's not clear to me why they should be given their presumed effect on productivity.
